...
impl TryInto<&'static dyn webpki::types::SignatureVerificationAlgorithm> for SignatureScheme {
    type Error = TlsError;
    fn try_into(
        self,
    ) -> Result<&'static dyn webpki::types::SignatureVerificationAlgorithm, Self::Error> {
...
static ALL_SIGALGS: &[&dyn webpki::types::SignatureVerificationAlgorithm] = &[
...
fn verify_signature(
    ...
    if !certificate.entries.is_empty() {
        ...
        if let CertificateEntryRef::X509(certificate) = certificate.entries[0] {
            let cert = CertificateDer::from_slice(certificate);
...
fn verify_certificate(
    ...
    if let Some(Certificate::X509(ca)) = ca {
        let ca_der = CertificateDer::from_slice(ca);
        ...
        if !certificate.entries.is_empty() {
            // TODO: Support intermediates...
            if let CertificateEntryRef::X509(certificate) = certificate.entries[0] {
                let certificate_der = CertificateDer::from_slice(certificate);
                let cert = webpki::EndEntityCert::try_from(&certificate_der).map_err(|e| {
                    warn!("Error loading cert: {:?}", e);
                    TlsError::DecodeError
                })?;

                let time = if let Some(now) = now {
                    webpki::types::UnixTime::since_unix_epoch(Duration::from_secs(now))
                } else {
                    // If no clock is provided, the validity check will fail
                    webpki::types::UnixTime::since_unix_epoch(Duration::from_secs(0))
                };
                ...
                match cert.verify_for_usage(
                    ...
                    Ok(_verified_path) => verified = true,
                    Err(e) => {
                ...
                if let Some(server_name) = verify_host {
                    match ServerName::try_from(server_name.as_bytes()) {
                        Ok(server_name) => {
                            match cert.verify_is_valid_for_subject_name(&server_name) {
